...
#### 5.  from the scatter with marginal plot-- CADD and Rankscore have:A strong positive non-linear correlation (rho = 0.72, p = 0) by Spearman
# Calculate Spearman's correlation and p-value
test_result <- cor.test(df$RankScore, df$CADD, method = "spearman")

# Extract rho (correlation coefficient) and p-value
rho <- test_result$estimate
p_value <- test_result$p.value

# Print the results
cat(paste("A rough positive non-linear correlation (rho = ", round(rho, 2), ", p = ", round(p_value, 5), ").", sep = ""))
