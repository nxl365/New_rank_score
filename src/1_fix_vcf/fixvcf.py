"""This script is for keeping the original records of a VCF file and adding some formatted values to a new fixed_vcf file,
 for the pre-requirement of the MIP annotation pipeline."""
...
with pysam.VariantFile(in_file, "r") as vcf_in:
    ...
    with pysam.VariantFile(out_file, "w", header=vcf_in.header) as vcf_out:
        ...
        # add records:
        count_none_alt = 0  # count how many none in the alts

        for rec in vcf_in:

            # change all tuple alts to string, None to '.'
