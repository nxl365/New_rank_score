""" 
This script is to extract features from vcf.gz file, use fitted `preprocessor` and `model` to predict + Allele frequency filter, write output back to vcf.gz file.

MODEL: LG4 : LG3 model trained on imbalanced classes CJP( Clinvar-Justhusky-KI pathogenic) with 12 features + AF filter,    preprocessor3
...
pred_out_simple = '/Users/nancy/Desktop/RS_projects/result/05_summary/01_12feats_CJP_justhusky_pred_out_simple.csv'   ## prediction and score of LG3 , a simple csv
pred_out_simple_after_AFfilter = '/Users/nancy/Desktop/RS_projects/result/05_summary/01_12feats_CJP_justhusky_pred_out_simple_after_AFfilter.csv'   ##  prediction and score after AF filter

pred_out_file = '/Users/nancy/Desktop/RS_projects/result/05_summary/01_12feats_CJP_justhusky_pred_out.vcf.gz'       ## final output, justhusky.vcf.gz with prediction and score


##### load fitted preprocessor and model
preprocessor = load('/Users/nancy/Desktop/RS_projects/result/04_ML_modify/01_retrainModel_ClinvarJusthuskyNewpatho/models/02_12feats_ClinJustPatho5-5_preprocessor.joblib') # preprocessor3
LG_model = load('/Users/nancy/Desktop/RS_projects/result/04_ML_modify/01_retrainModel_ClinvarJusthuskyNewpatho/models/02_12feats_ClinJustPatho5-5_LG.joblib')   # LG3
...
print(" 3. finsh: Allele frequency filtering")





""" ##  4. Write output back in vcf.gz ## """
    ...
    with gzip.open(pred_out_file,'wb') as out_f1:           # decode and encode: for compressed file
        ...
        for i,line in enumerate(in_f,0):                           
            ...
            if content.startswith('##INFO=<ID=RankResult'):
                ...
                score = '##INFO=<ID=LGrank_score,Number=.,Type=String,Description="The rank score for this variant, scores range from 0 to 1, the larger the score the more likely it is to be pathogenic.">\n'
                out_f1.write((pre + score).encode())



print(" 4. finish: Write output back in vcf.gz")
