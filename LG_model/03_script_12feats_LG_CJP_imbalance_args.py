""" 
This script is to extract features from annotated vcf.gz file, use fitted `preprocessor` and `model` to predict + Allele frequency filter, write output back to vcf.gz file.
...
--model /Users/nancy/Desktop/RS_projects/result/04_ML_modify/01_retrainModel_ClinvarJusthuskyNewpatho/models/02_12feats_ClinJustPatho5-5_LG.joblib 
--pre /Users/nancy/Desktop/RS_projects/result/04_ML_modify/01_retrainModel_ClinvarJusthuskyNewpatho/models/02_12feats_ClinJustPatho5-5_preprocessor.joblib 
--info ttt_info.csv 
--out ttt_out.vcf.gz
...
try:
    ...
    print ('predict.py --in <in_vcf.gz> --cons <variants_consequences.txt> --model <fitted_model.joblib> --pre <fitted_preprocessor.joblib> --info <INFO_extracted.csv> --out <out_vcf.gz> ')
for opt, arg in opts:
    if opt =='--in':
        vcfgz_in_file = arg
